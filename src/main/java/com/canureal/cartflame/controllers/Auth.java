...
public class Auth {
    ...
    @PostMapping("/signup")
    UserCrudDto.RegisterRequestDto signup(@Valid @RequestBody UserCrudDto.RegisterRequestDto dto) {
        userCrudService.registerUser(dto);
        return dto;
    }

    @PostMapping("/signin")
    public ResponseEntity<UserCrudDto.LoginResponseDto> signin(@Valid @RequestBody UserCrudDto.LoginRequestDto dto, HttpServletResponse response) {
